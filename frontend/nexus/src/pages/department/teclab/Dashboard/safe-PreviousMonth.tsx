...
function EngineeringPreviousMonth({ responseData }) {
    ...
    const { getColorForName } = useColors();
    ...
    useEffect(() => {
        ...
        if (responseData && responseData["PREVIOUS_MONTH"] && responseData["PREVIOUS_MONTH"]["PIECHARTDATA"]) {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                // First check if the data exists before trying to access it
                if (!responseData || !responseData.PREVIOUS_MONTH || !responseData.PREVIOUS_MONTH.BARCHARTDATA) {
                    console.log("Data not yet available or missing expected structure");
                ...
                processedData.map((entry, idx) => (
                    ...
                    processedData[idx].colorHexCode = getColorForName(entry.engineer)
                ))

                setSubChart2Data(processedData);
            } catch (error) {
                console.error("Failed to fetch data in the above useEffect:", error);
            }
        }
        fetchData();
    }, [responseData]);
    ...
    return (
        <Card className="w-max mx-4">
            <CardHeader className="text-center text-xl p-2"> {responseData["PREVIOUS_MONTH"]["VALUE"]} </CardHeader>
            <CardContent className="flex items-center justify-center">
                ...
                <PieChart width={600} height={400} className="p-0 m-0">
                    <Pie
                        ...
                        legendType="square"
                    >
                        {Object.entries(responseData["PREVIOUS_MONTH"]["PIECHARTDATA"]).map(([key, value], index) => (
                            // <Cell key={`cell-${key}`} fill={COLORS[index % COLORS.length]} />
                            <Cell key={`cell-${key}`} fill={getColorForName(key)} />
                        ))}
                    </Pie>
